...
const baseArticles: BlogArticle[] = [
    ...
    define('how-to-protect-a-pdf-with-password', {
        introduction: [
            ...
            'Protect PDF in PDF by ib applies AES-256 password encryption locally in your browser for supported unencrypted PDFs. This guide explains how to use it safely and verify the protected result.',
        ],
        sections: [
            ...
            { id: 'current-status', heading: 'Current PDF by ib support', paragraphs: ['Protect PDF accepts supported unencrypted PDFs and applies AES-256 password encryption entirely in the browser. The document and password are not sent to a processing server. Already encrypted PDFs cannot currently be unlocked, and some unusual PDF structures may be incompatible.', 'After downloading, test the protected result in at least one separate PDF viewer. Confirm that an incorrect password fails, the correct password opens the document, and every page is present.', 'Keep the protected and unprotected copies in clearly separated approved locations. Use a filename that does not reveal sensitive content, and send only the intended version. If access must later be revoked or audited, ordinary PDF passwords may be insufficient; use an organizational document-sharing system with identity and access controls instead.'] },
        ],
        faq: [
            { question: 'Can PDF by ib protect a PDF today?', answer: 'Yes. Protect PDF applies AES-256 password encryption locally to supported unencrypted PDFs.' },
...
const seoIntentDetails: Record<string, { keyword: string; quickAnswer: string; mistakes: string[] }> = {
    ...
    'how-to-protect-a-pdf-with-password': { keyword: 'protect pdf', quickAnswer: 'Choose a supported unencrypted PDF, enter a strong unique password twice, protect a copy, and test the download in another viewer. Send the password through a different channel.', mistakes: ['Sending the password in the same message as the file.', 'Using a short or reused password.', 'Assuming password protection permanently prevents copying.', 'Losing the only unprotected original.'] },
