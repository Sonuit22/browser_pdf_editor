...
import { formatSigningDate, renderSigningVisual } from '../modules/pdf/editor/utils/signingVisual';
...
export const RightPanel = memo(function RightPanel() {
    ...
    const updateDate = (dateValue: string) => {
        if (!selected || selected.type !== 'signature' || selected.signatureKind !== 'date' || !dateValue) return;
        const visual = renderSigningVisual(formatSigningDate(dateValue), { width: 560, height: 160, font: '600 64px Arial, sans-serif' });
        editor.update(selected.id, { source: visual.source, aspectRatio: visual.aspectRatio, dateValue });
    };
    return <aside className={`right-panel document-details${open ? ' is-open' : ''}${selected ? ' has-selection' : ''}`} aria-label="Document details">
        ...
        {open && <div className="document-details__body">
            ...
            {selected && <section className="selected-properties"><h3>Selected {selected.type.replace('-', ' ')}</h3>
                ...
                {selected.type === 'signature' && selected.signatureKind === 'date' && <label>Date<input type="date" value={selected.dateValue ?? new Date().toISOString().slice(0, 10)} onChange={(event) => updateDate(event.target.value)} /></label>}
                ...
                {selected.type === 'signature' && <p className="property-note">{selected.signatureKind === 'date' || selected.signatureKind === 'checkmark' ? `Signing object: ${selected.signatureKind}.` : `Signature type: ${selected.signatureKind}.`} Stored only for this editing session.</p>}
