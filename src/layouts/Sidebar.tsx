...
export const Sidebar = memo(function Sidebar({ onNavigate, onClose }: { onNavigate: (destination: string) => void; onClose: () => void }) {
    ...
    return <aside ref={aside} id="tool-sidebar" className="sidebar" aria-label="PDF tools">
        ...
        <nav>{toolRegistry.map((item) => { const Icon = item.icon; return <NavLink key={item.id} to={item.route} onClick={(event) => { event.preventDefault(); onNavigate(item.route); }} aria-label={item.title} data-tooltip={item.title}><Icon size={21} strokeWidth={1.8} aria-hidden="true" /><span>{item.title}</span></NavLink>; })}</nav>
    </aside>;
});
