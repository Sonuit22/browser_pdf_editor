...
export function PdfEngineProvider({ children }: { children: ReactNode }) {
    ...
    return <PdfEngineContext.Provider value={value}><input ref={inputRef} className="sr-only" type="file" accept="application/pdf,.pdf" onChange={onFileChange} tabIndex={-1} aria-hidden="true" />{children}</PdfEngineContext.Provider>;
}
