...
export function EditorToolbar({ onExport, exporting }: { onExport: () => void; exporting: boolean }) {
    ...
    const highlighterPopover = highlighterOpen && isHighlighterActive && typeof document !== 'undefined'
        ? createPortal(<section id="highlighter-settings-popover" className="highlighter-settings" aria-label="Highlighter settings" data-highlighter-controls role="dialog" style={{ left: highlighterPosition.left, top: highlighterPosition.top, width: Math.min(POPOVER_WIDTH, window.innerWidth - 24) }}>
            ...
            })}<label className="custom-color" title="Custom highlight color"><span>Custom</span><input type="color" aria-label="Custom highlight color" value={normalizedHighlighter.color} onInput={(event) => updateHighlighter({ color: event.currentTarget.value })} /></label></div></fieldset>
...
function ToolSettingsPopover() {
    ...
    return createPortal(<section className="tool-settings-popover" role="dialog" aria-label={`${kind} settings`} data-tool-settings style={{ left: position.left, top: position.top, width: Math.min(POPOVER_WIDTH, window.innerWidth - 24) }}>
        {kind === 'text' && <><strong>Text settings</strong><label>Color <input type="color" value={editor.textSettings.color} onInput={(event) => editor.updateTextSettings({ color: event.currentTarget.value })} /></label><label>Font size <output>{editor.textSettings.fontSize}px</output><input type="range" min="6" max="72" value={editor.textSettings.fontSize} onChange={(event) => editor.updateTextSettings({ fontSize: Number(event.target.value) })} /></label></>}
        {kind === 'draw' && <><strong>Draw settings</strong><label>Color <input type="color" value={editor.drawSettings.color} onInput={(event) => editor.updateDrawSettings({ color: event.currentTarget.value })} /></label><label>Stroke width <output>{editor.drawSettings.strokeWidth}px</output><input type="range" min="1" max="20" value={editor.drawSettings.strokeWidth} onChange={(event) => editor.updateDrawSettings({ strokeWidth: Number(event.target.value) })} /></label><label>Opacity <output>{Math.round(editor.drawSettings.opacity * 100)}%</output><input type="range" min="10" max="100" value={Math.round(editor.drawSettings.opacity * 100)} onChange={(event) => editor.updateDrawSettings({ opacity: Number(event.target.value) / 100 })} /></label></>}
        {kind === 'shape' && <><strong>Shape settings</strong><div className="shape-options">{shapeTools.map(([tool, label]) => <button type="button" className={editor.activeTool === tool ? 'is-active' : ''} key={tool} onClick={() => editor.setTool(tool)}>{label}</button>)}</div><label>Stroke <input type="color" value={editor.shapeSettings.strokeColor} onInput={(event) => editor.updateShapeSettings({ strokeColor: event.currentTarget.value })} /></label><label>Border width <output>{editor.shapeSettings.strokeWidth}px</output><input type="range" min="1" max="12" value={editor.shapeSettings.strokeWidth} onChange={(event) => editor.updateShapeSettings({ strokeWidth: Number(event.target.value) })} /></label><label>Opacity <output>{Math.round(editor.shapeSettings.opacity * 100)}%</output><input type="range" min="10" max="100" value={Math.round(editor.shapeSettings.opacity * 100)} onChange={(event) => editor.updateShapeSettings({ opacity: Number(event.target.value) / 100 })} /></label></>}
    </section>, document.body);
}
