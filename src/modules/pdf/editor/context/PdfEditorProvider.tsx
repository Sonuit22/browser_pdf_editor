...
export function PdfEditorProvider({ children }: { children: ReactNode }) {
    ...
    const hydrateForm = useCallback((annotationsByPageId: Record<string, PdfAnnotation[]>, formValues: EditorPresent['formValues']) => dispatch({ type: 'hydrate-form', annotationsByPageId, formValues }), []);
    const clearForm = useCallback(() => {
        const present = stateRef.current.present;
        const annotationsByPageId = Object.fromEntries(Object.entries(present.annotationsByPageId).map(([pageId, annotations]) => [pageId, annotations.filter((annotation) => annotation.native)]));
        const formValues: EditorPresent['formValues'] = {};
        for (const annotation of Object.values(annotationsByPageId).flat()) {
            if (!('name' in annotation)) continue;
            formValues[annotation.name] = annotation.type === 'form-checkbox' ? false : annotation.type === 'form-choice' ? [] : '';
        }
        commit({ ...present, annotationsByPageId, formValues, selectedId: null, selectedIds: [], activeTool: 'select' });
    }, [commit]);
    ...
    const value = useMemo(() => ({ activeTool: state.present.activeTool, highlighterSettings: state.present.highlighterSettings, textSettings: state.present.textSettings, drawSettings: state.present.drawSettings, shapeSettings: state.present.shapeSettings, selectedId: state.present.selectedId, selectedIds: state.present.selectedIds, annotationsByPageId: state.present.annotationsByPageId, formValues: state.present.formValues, flattenForms: state.present.flattenForms, layout: state.present.layout, history: state.past, dirty: state.present.dirty, canUndo: state.past.length > 0, canRedo: state.future.length > 0, setTool: (tool: EditorTool) => dispatch({ type: 'tool', tool }), updateHighlighterSettings: (settings: Partial<EditorPresent['highlighterSettings']>) => dispatch({ type: 'highlighter-settings', settings }), updateTextSettings: (settings: Partial<EditorPresent['textSettings']>) => dispatch({ type: 'text-settings', settings }), updateDrawSettings: (settings: Partial<EditorPresent['drawSettings']>) => dispatch({ type: 'draw-settings', settings }), updateShapeSettings: (settings: Partial<EditorPresent['shapeSettings']>) => dispatch({ type: 'shape-settings', settings }), select: (id: string | null, append = false) => dispatch({ type: 'select', id, append }), selectMany: (ids: string[]) => dispatch({ type: 'select-many', ids }), add, update, remove, removeSelected, duplicate, duplicateSelected, alignSelected, updateLayout, setFormValue, hydrateForm, clearForm, setFlattenForms, undo: () => dispatch({ type: 'undo' }), redo: () => dispatch({ type: 'redo' }), copy, paste, reorder }), [add, alignSelected, clearForm, copy, duplicate, duplicateSelected, hydrateForm, paste, remove, removeSelected, reorder, setFlattenForms, setFormValue, state, update, updateLayout]);
