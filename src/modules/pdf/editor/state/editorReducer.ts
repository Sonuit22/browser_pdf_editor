...
export function editorReducer(state: EditorHistoryState, action: EditorAction): EditorHistoryState {
    ...
    if (action.type === 'hydrate-form') return { present: { ...state.present, annotationsByPageId: action.annotationsByPageId, formValues: action.formValues, selectedId: null, selectedIds: [], dirty: false }, past: [], future: [] };
