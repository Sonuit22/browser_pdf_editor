...
export function createAnnotation(tool: EditorTool, pageId: string, point: Point, highlighter: HighlighterSettings = { color: '#ffe066', opacity: .3, strokeWidth: 20 }): PdfAnnotation | null {
    ...
    if (tool === 'highlight') return { ...base(pageId, 'highlight', point), type: 'highlight', color: highlighter.color, opacity: highlighter.opacity, strokeWidth: highlighter.strokeWidth, points: [point] };
