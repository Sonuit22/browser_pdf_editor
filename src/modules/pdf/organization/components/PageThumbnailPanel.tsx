...
export function PageThumbnailPanel({ pages, activePageId, selectedPageIds = [], getPage, onSelect, onReorder, onRotate, onDuplicate, onDelete, selectionEnabled = false, reorderEnabled = false, label = 'Document pages', layout = 'strip' }: Props) {
    ...
    return <div className={`page-thumbnail-panel page-thumbnail-panel--${layout}`} role="list" aria-label={label}>
        {pages.map((page, index) => {
            ...
            return <article key={page.id} data-page-id={page.id} className={`shared-page-thumbnail${selected ? ' is-selected' : ''}${target}`} role="listitem" draggable={reorderEnabled}
                onDragStart={() => { dragging.current = page.id; }} onDragOver={(event: DragEvent<HTMLElement>) => { if (!reorderEnabled) return; event.preventDefault(); setDrop({ id: page.id, placement: placementFor(event.currentTarget, event.clientX, event.clientY) }); }} onDrop={(event) => { event.preventDefault(); finish(page.id, drop?.placement ?? 'before'); }}>
                {reorderEnabled && <button className="page-drag-handle" type="button" aria-label={`Drag page ${index + 1}`} title="Drag to reorder" onPointerDown={(event) => { dragging.current = page.id; event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={pointerMove} onPointerUp={(event) => { if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); if (drop) finish(drop.id, drop.placement); }}><GripVertical size={18} /></button>}
                {selectionEnabled && <input type="checkbox" checked={selected} onChange={() => onSelect(page.id, 'toggle')} aria-label={`Select page ${index + 1}`} />}
                <PdfThumbnail page={page} pageNumber={index + 1} active={activePageId === page.id} rotation={0} getPage={getPage} onSelect={choose} />
