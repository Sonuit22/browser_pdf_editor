...
export function validatePdfFileSelection(file: File) {
...
export async function validatePdfFile(file: File) {
    ...
    try {
        const document = await PDFDocument.load(data.slice(), { ignoreEncryption: true, throwOnInvalidObject: true, updateMetadata: false });
        if (document.isEncrypted) throw new Error('PASSWORD_PROTECTED_PDF');
    } catch (error) {
        if (error instanceof Error && error.message === 'PASSWORD_PROTECTED_PDF') {
            throw new Error('This PDF is password protected and cannot be processed without its password.');
        }
        throw new Error('The selected file is corrupted or is not a readable PDF.');
    }
    return data;
}
