...
import type { UtilityApplicationMode, UtilityPosition, UtilityTarget } from '../types/utilities';
import { applicationModeLabel, resolveUtilityTarget, type ResolvedUtilityTarget } from '../utils/utilityTargeting';
...
export function UtilityWorkspace() {
    ...
    const applyCrop = () => utilities.applyCrop(pages.map((page) => page.id), utilities.cropsByPageId[activePageId ?? ''] ?? { left: 0, right: 0, top: 0, bottom: 0 });
    ...
    const crop = utilities.cropsByPageId[activePageId ?? ''] ?? { left: 0, right: 0, top: 0, bottom: 0 };
    return <section className="utility-workspace" aria-label="PDF utility tools">
        ...
        <div className="utility-grid">
            <UtilitySection icon={Stamp} title="Watermark">
                <TargetControls id="watermark-target" target={utilities.watermark} resolution={watermarkTarget} onChange={utilities.updateWatermark} />
                ...
                <p className="utility-note">Watermarks are placed above PDF content and below local annotations for reliable browser exports.</p>
            ...
            <UtilitySection icon={Hash} title="Page numbers">
                <TargetControls id="page-numbers-target" target={utilities.pageNumbers} resolution={pageNumberTarget} onChange={utilities.updatePageNumbers} />
            ...
            <UtilitySection icon={Tags} title="Header and footer">
                <TargetControls id="header-footer-target" target={utilities.headerFooter} resolution={headerFooterTarget} onChange={utilities.updateHeaderFooter} />
            ...
            <UtilitySection icon={Crop} title="Crop pages"><p className="utility-note">Crop clips the exported page. It does not zoom the viewer or move existing annotations.</p><div className="utility-inline">{(['left', 'right', 'top', 'bottom'] as const).map((side) => <label key={side}>{side}<input type="number" min="0" max="300" value={crop[side]} onChange={(event) => utilities.applyCrop([activePageId ?? ''], { ...crop, [side]: Math.max(0, Number(event.target.value)) })} /></label>)}</div><Actions onApply={applyCrop} onReset={() => utilities.resetCrop(pages.map((page) => page.id))} /></UtilitySection>
