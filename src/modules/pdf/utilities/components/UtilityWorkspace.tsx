...
import { Crop, Hash, ImagePlus, RotateCw, Stamp, Tags } from 'lucide-react';
...
export function WatermarkSection({ onApplied }: { onApplied?: () => void } = {}) {
    ...
    const updateImage = async (event: ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            utilities.updateWatermark({ kind: 'image', imageSource: image.data, imageWidth: image.width, imageHeight: image.height });
        } catch (error) {
