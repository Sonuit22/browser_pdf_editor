...
export type WatermarkSettings = UtilityTarget & { enabled: boolean; kind: 'text' | 'image'; text: string; imageSource: string | null; imageWidth: number | null; imageHeight: number | null; position: UtilityPosition; x: number; y: number; fontSize: number; color: string; opacity: number; rotation: number };
...
export const defaultUtilitySettings: UtilitySettings = {
    watermark: { enabled: false, pageIds: [], applicationMode: 'all', customRange: '', kind: 'text', text: 'CONFIDENTIAL', imageSource: null, imageWidth: null, imageHeight: null, position: 'center', x: 50, y: 50, fontSize: 42, color: '#0f6aa6', opacity: .24, rotation: 45 },
