...
import type { HighlightAnnotation, ImageAnnotation, ShapeAnnotation, SignatureAnnotation, TextAnnotation } from '../src/modules/pdf/editor/types/annotations';
...
describe('editor export', () => {
    it('embeds locally added images and supported shapes into a valid PDF', async () => {
        ...
        const bytes = await createWorkingPdf({ pages: [page], annotationsByPageId: { [page.id]: [image, signature, text, highlight, ...shapes] }, getSourceFile: () => null });
