...
import { absoluteSiteUrl, normalizeCanonicalPath, SITE_URL } from '../src/config/site';
...
const pagePaths = [
    ...
    '/ppt-to-pdf', '/protect-pdf', '/unlock-pdf', '/translate-pdf', '/fill-pdf-form', '/watermark-pdf', '/image-resizer', '/sign-pdf', '/edit-pdf', '/faq', '/privacy', '/terms', '/about',
    '/contact', '/support', '/blog', '/missing',
];

describe('route SEO configuration', () => {
    it.each(['/', '/all-tools', '/merge-pdf', '/split-pdf', '/remove-pages', '/extract-pages', '/organize-pdf', '/jpg-to-pdf', '/pdf-to-jpg', '/protect-pdf', '/unlock-pdf', '/fill-pdf-form', '/watermark-pdf', '/image-resizer', '/sign-pdf', '/edit-pdf', '/faq', '/privacy', '/terms', '/about', '/contact'])('provides useful metadata for %s', (path) => {
    ...
    it('creates one normalized production canonical origin', () => {
        ...
        expect(absoluteSiteUrl('/image-resizer?utm_source=test#settings')).toBe(`${SITE_URL}/image-resizer`);
        expect(normalizeCanonicalPath('//blog///how-to-sign-a-pdf-online/?ref=test#faq')).toBe('/blog/how-to-sign-a-pdf-online');
    });
